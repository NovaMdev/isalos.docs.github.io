...
A data table in long format consisting of two categorical or numerical columns that represent the source and target nodes, respectively. A numerical column of link values should also be included to show relationship magnitudes. If necessary, the conversion from wide to long format can be achieved via the `Wide to Long` function that can be found in the `Data Transformation` menu of the tools ribbon. 

### Configuration
{: .no_toc}

| **Chart Title**           | Type the chart title.|      
| **Source Category**| Select the source nodes column which can be either categorical or numerical.  |  
| **Target Category** | Select the target nodes column which can be either categorical or numerical. |
...
In the left-hand spreadsheet, import a table in long format including two categorical or numerical columns which correspond to the source and target categories, respectively, and a numerical column consisting of the link values (1).
...
| **Show Node Sum**  | Display the total of the values for all links terminating at each node.|   
| **Show Link Values** | Display numerical link values on the chart. |    
| **Background**      | Select the background color. |   
